...
%% --- REA (serie alternativa alla OCSE) ---
opts_rea = detectImportOptions('rea.csv', ...
    'TextType','string', ...
    'VariableNamingRule','preserve');

% prendiamo semplicemente le prime 2 colonne (qualunque sia il nome)
opts_rea.SelectedVariableNames = opts_rea.VariableNames(1:2);

rea = readtable('rea.csv', opts_rea);

% rinomino esplicitamente
rea.Properties.VariableNames = {'Date','Value'};

% timetable
REA = timetable(rea.Date, rea.Value, 'VariableNames', {'REA'});
REA = sortrows(REA);
REA = REA(timerange(startD,endD,'closed'),:);
...
% parsing date
rea.Date = datetime(string(rea.Date), 'InputFormat','yyyy-MM-dd');
...
All = synchronize(WTI, CPI, OCSE, REA, INV, PRD, FFR, JET, 'intersection');
...
rea_d = diff(All.REA);  % come OCSE: niente log



dates_d = All.Time(2:end);

All_d = timetable(dates_d, wti_d, prod_d, ocse_d, rea_d, inv_d, jet_d, ...
    'VariableNames', {'WTI_real_DL','Production_DL','OCSE_DL','REA_DL', 'Inventories_DL', 'JeTFuel_DL'});
