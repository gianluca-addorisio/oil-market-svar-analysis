...
% Target di partenza "realistico" in USD (es. 65 USD al barile)
P0_USD = 55;
...
baseline_plot = plot(t, P_baseline_USD, ':', 'Color','k', ...
    'LineWidth', 1.2);             % più sottile e puntinata

scenario_plot = plot(t, P_scenario_USD, 'k-', ...
    'LineWidth', 2.0);             % più spessa
...
% --- Range ---
ymin = min([P_baseline_USD; P_scenario_USD]) - 1;
ymax = max([P_baseline_USD; P_scenario_USD]) + 1;
ylim([ymin, ymax]);

xlim([0, H-1]);
% --- Tick personalizzato per la baseline ---
baseline_val = P0_USD;              % livello della baseline (es. 65 USD)

% Tick attuali
ticks = yticks;

% Se il valore baseline non è presente, lo aggiungo e ordino
if ~ismember(baseline_val, ticks)
    ticks = [ticks baseline_val];
    ticks = unique(ticks);          % unique li rende anche crescenti
    yticks(ticks);
end

% Ricarico tick aggiornati
ticks = yticks;
labels = string(ticks);

% Trovo il tick corrispondente alla baseline (con tolleranza numerica)
idx = find(abs(ticks - baseline_val) < 1e-6);

% Cambio SOLO quell'etichetta
labels(idx) = sprintf('%.0f*', baseline_val);    % oppure '65 (baseline)'
